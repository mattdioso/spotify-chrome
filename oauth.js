function startSpotifyOAuth(success_function) {
	var client_id = 'ecbbd1a3f1b148019e8f7454e0f89205';
	var client_secret = '409ef0fc3a4743b89c3d9a2e35d59f39';
	...
	chrome.identity.launchWebAuthFlow({
		...
		"&response_type=code&scope=app-remote-control%20playlist-read-private%20play-list-modify-private%20playlist-modify-public%20user-library-read%20playlist-modify%20user-modify-playback-state",
		'interactive': true,
	},
		function(redirect_url) {
			...
			for (var parameterNumber in queryParameters) {
				var parameter = queryParameter[parameterNumber];
	...
	req.onreadystatechange = function() {
		if (this.readyState == XMLHttpRequest.DONE && this.status ==200) {
			...
			chrome.storage.local.set({'spotify_auth':spotify_auth}, function() {
				internal_function(spotify['access_token']);
			});
		}
	}
