...
class SpotifySearch {
	...
	results(results_call_back) {
		...
		var getRequest = new SpotifyGet("/v1/search?type=" + this.type + "&q=" + textSearch);
...
class AddTracksToPlaylist {
	...
	request(uris, success_function) {
		var postRequest = new SpotifyPost("/v1/playlists/" + this.playlist_id + "/tracks", {uris:uris});
...
class CreatePlaylist {
	...
	results(success_function) {
		...
		var postRequest = new SpotifyPost("/v1/me/playlists", {name:"Spotify Select", public:false});
		postRequest.results(function(responseText) {
			var json = JSON.parse(responseText);
...
class AlbumTrackList {
	...
	results(success_function) {
		var getRequest = new SpotifyGet("/v1/albums/" + this.album_id + "/tracks");
		getRequest.results(function(responseText) {
			var json = JSON.parse(responseText);
...
class FindPlayList {
	...
	results(found, failed) {
		var playlist_name = this.name;
		var getRequest = new SpotifyGet("/v1/me/playlists");
		getRequest.results(function(responseText) {
			var json = JSON.parse(responseText);
...
class DetailedTrackList {
	...
	results(success_function) {
		var getRequest = new SpotifyGet("/v1/tracks?ids=" + this.track_ids.join(","));
		getRequest.results(function(responseText) {
			var json = JSON.parse(responseText);
			success_function(json['track']);
		});
	}
}
